...
export const getItemList = (path) => {
	path = fixPath(path);
	return API.readFolder(path).then(({ files, folders }) => [ ...files, ...folders ]).catch(logFetchError);
};
...
export const getFileBlob = (path, filename) => {
	path = fixPath(path);
	return API.fetchFile(path, filename).then((response) => response.blob()).catch(logFetchError);
};
...
export const renameFile = (path, fileName, newFileName) => {
	path = fixPath(path);
	return API.renameFile(path, fileName, newFileName).catch(logFetchError);
};
...
export const renameFolder = (path, folderName, newFolderName) => {
	path = fixPath(path);
	return API.renameFolder(path, folderName, newFolderName).catch(logFetchError);
};
...
export const removeItems = (path, filenames) => {
	...
	return API.removeItems(path, filenames).catch(logFetchError);
};
...
export const moveItems = (path, destination, filenames) => {
	...
	return API.moveItems(path, destination, filenames).catch(logFetchError);
};
...
export const copyItems = (path, destination, filenames) => {
	...
	return API.copyItems(path, destination, filenames).catch(logFetchError);
};
...
export const uploadFiles = (path, fileList) => {
	...
	return API.upload(path, fileList).catch(logFetchError);
};
...
export const updateFile = (path, fileName, content) => {
	path = fixPath(path);
	return API.updateItem(path, fileName, content).catch(logFetchError);
};
...
export const getAsZip = (path, itemList) => {
	...
	return addItemsToZip(zip, path, itemList).then(() => zip);
};
...
async function uploadExtractedZipArchive(zip, destination, curFolder = '') {
	const promises = getItemsInZipFolder(zip, curFolder).map(async (item) => {
